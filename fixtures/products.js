...
const subcategoryToCategory = data.subcategories.reduce((map, subcategory) => {
  if (subcategory.count > 0 && activeCategories.has(subcategory.category)) {
    map[subcategory.id] = subcategory.category;
  }
...
module.exports = data.products
  ...
  .filter(product => !!subcategoryToCategory[product.subcategory])
  .map(product => {
    ...
    return {
      ...
      category: subcategoryToCategory[product.subcategory] || null,
